...
class APIService:
    ...
    def __init__(self):
        ...
        self.base_url = 'https://e6db2f06-15c4-4633-bd30-7fbd9c8200b1-00-l2xqyupphiyt.riker.replit.dev'
    ...
    def _handle_error(self, error: Exception, endpoint: str = "unknown") -> Dict[str, Any]:
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.request(
                        ...
                        Logger.info(f'API Response: {response.status} from {endpoint}')
                        ...
                        try:
                            ...
                            raise APIConnectionError(f"Invalid JSON response from {endpoint}")
                        ...
                        if response.status >= 400:
                            ...
                            if response.status == 401:
                                ...
                                raise APIConnectionError(f"API error {response.status}: {error_msg}")
                ...
                if attempt < retries:
                    ...
                    raise APIConnectionError(f"Failed after {retries + 1} attempts: {str(e)}")
    ...
    async def check_api_status(self) -> bool:
        ...
        try:
            response = await self._make_request('GET', '/api/health', retries=1)
            return True
        except Exception as e:
    ...
    async def load_saved_credentials(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if saved_phone and saved_password:
                Logger.info('Znaleziono zapisane dane logowania, inicjalizacja sesji...')
                self.initialize(saved_phone, saved_password)
                return {"phone": saved_phone, "password": saved_password}
            return None
        except Exception as error:
            Logger.error(f'Błąd podczas ładowania zapisanych danych logowania: {error}')
            return None
    ...
    async def auto_login(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if credentials:
                ...
                return await self.login(credentials["phone"], credentials["password"])
            return {"success": False, "message": "Brak zapisanych danych logowania"}
        except Exception as error:
            Logger.error(f'Błąd podczas autologowania: {error}')
            return {"success": False, "message": f"Błąd podczas autologowania: {error}"}
    
    async def save_credentials(self, phone: str, password: str, base_url: str = None):
        ...
        try:
            ...
            Logger.error(f'Błąd podczas zapisywania danych logowania: {error}')
    ...
    async def load_saved_api_url(self) -> Optional[str]:
        ...
        try:
            ...
            Logger.error(f'Błąd podczas wczytywania zapisanego adresu API: {error}')
            return None
    ...
    def initialize(self, phone: str, password: str, base_url: str = None):
        ...
        self.base_url = base_url or 'https://e6db2f06-15c4-4633-bd30-7fbd9c8200b1-00-l2xqyupphiyt.riker.replit.dev'
        ...
        Logger.info(f'API Service initialized with phone: {phone} and baseUrl: {self.base_url}')
    
    def get_base_url(self) -> str:
        """Pobieranie bazowego adresu URL API"""
        if not self.base_url:
            Logger.warning('baseUrl nie jest zdefiniowany, używam domyślnego')
            return 'https://e6db2f06-15c4-4633-bd30-7fbd9c8200b1-00-l2xqyupphiyt.riker.replit.dev'
    ...
    async def _auth_fetch(self, endpoint: str, options: Dict = None) -> Dict:
        ...
        try:
            if not self.is_logged_in:
                Logger.warning('Użytkownik nie jest zalogowany. Próba użycia _auth_fetch bez logowania.')
    ...
    # === LOGOWANIE ===
    async def login(self, phone: str, password: str, base_url: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not login_base_url:
                login_base_url = 'https://e6db2f06-15c4-4633-bd30-7fbd9c8200b1-00-l2xqyupphiyt.riker.replit.dev'
    ...
    async def logout(self) -> Dict[str, Any]:
        ...
        try:
            if self.is_logged_in:
                await self._auth_fetch('/api/driver2/logout', {'method': 'POST'})
    ...
    async def check_order_pool(self) -> Dict[str, Any]:
        ...
        """
        try:
            Logger.info('Sprawdzanie puli zleceń dla testowego kierowcy (ID: 15)...')
            ...
            if response.get('success') and isinstance(response.get('data'), list):
                Logger.info(f'Znaleziono {len(response["data"])} zleceń w puli kierowcy')
                return {"success": True, "data": response['data']}
            elif response.get('status') == "success" and isinstance(response.get('orders'), list):
                Logger.info(f'Znaleziono {len(response["orders"])} zleceń w puli kierowcy')
                return {"success": True, "data": response['orders']}
            elif response.get('success') and response.get('orders'):
                Logger.info('Format alternatywny - znaleziono zlecenia w puli kierowcy')
                return {"success": True, "data": response['orders']}
            else:
                Logger.info('Brak zleceń w puli lub nieobsługiwany format odpowiedzi')
    ...
    async def get_current_orders(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.get('success') and isinstance(response.get('data'), list):
                ...
                Logger.info('Brak aktualnych zleceń lub nieobsługiwany format odpowiedzi')
                return {"success": True, "data": []}
        except Exception as error:
            Logger.error(f'Błąd podczas pobierania aktualnych zleceń: {error}')
            return {"success": False, "message": str(error), "data": []}
    ...
    async def accept_order(self, order_id: int) -> Dict[str, Any]:
        ...
        """
        try:
            response = await self._auth_fetch(f'/api/driver2/orders/{order_id}/accept', {
                'method': 'POST'
            })
            ...
            if response.get('success'):
                ...
                return {"success": False, "message": response.get('message', 'Błąd akceptacji')}
        except Exception as error:
    ...
    async def start_order(self, order_id: int) -> Dict[str, Any]:
        ...
        """
        try:
            response = await self._auth_fetch(f'/api/driver2/orders/{order_id}/start', {
                'method': 'POST'
            })
            ...
            if response.get('success'):
                ...
                return {"success": False, "message": response.get('message', 'Błąd rozpoczęcia')}
        except Exception as error:
    ...
    async def complete_order(self, order_id: int) -> Dict[str, Any]:
        ...
        """
        try:
            response = await self._auth_fetch(f'/api/driver2/orders/{order_id}/complete', {
                'method': 'POST'
            })
            ...
            if response.get('success'):
                ...
                return {"success": False, "message": response.get('message', 'Błąd zakończenia')}
        except Exception as error:
    ...
    async def cancel_order(self, order_id: int, reason: str = "") -> Dict[str, Any]:
        ...
        """
        try:
            response = await self._auth_fetch(f'/api/driver2/orders/{order_id}/cancel', {
                'method': 'POST',
                'json': {"reason": reason}
            })
            ...
            if response.get('success'):
                ...
                return {"success": False, "message": response.get('message', 'Błąd anulowania')}
        except Exception as error:
    ...
    async def get_order_details(self, order_id: int) -> Dict[str, Any]:
        ...
        """
        try:
            response = await self._auth_fetch(f'/api/driver2/orders/{order_id}')
            ...
            Logger.error(f'Błąd pobierania szczegółów zlecenia {order_id}: {error}')
            return {"success": False, "message": str(error)}
    ...
    async def get_order_storage_details(self, order_id: int) -> Dict[str, Any]:
        ...
        """
        try:
            response = await self._auth_fetch(f'/api/driver2/order_storage/{order_id}')
            ...
            Logger.error(f'Błąd pobierania szczegółów zlecenia z magazynu {order_id}: {error}')
            return {"success": False, "message": str(error)}
    ...
    async def update_driver_status(self, status: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.get('success'):
                ...
                return {"success": False, "message": response.get('message', 'Błąd aktualizacji statusu')}
        except Exception as error:
            Logger.error(f'Błąd aktualizacji statusu: {error}')
            return {"success": False, "message": str(error)}
    
    # === LOKALIZACJA ===
    async def update_location(self, latitude: float, longitude: float) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if response.get('success'):
                ...
                return {"success": False, "message": response.get('message', 'Błąd aktualizacji lokalizacji')}
        except Exception as error:
